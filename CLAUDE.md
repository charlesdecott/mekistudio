...
- Câbles/wires **dérivés** d'un parent par node (`Node.source_id`). Parentage **par kind** (`CANONICAL_PARENT_KIND`) pour les built-in, **path-aware** (plus-long-préfixe de chemin, `nodes/parenting.py` pur) pour `folder`/`fileeditor` ; `reconcile_source_links` applique les deux au boot (+ migration des built-in de mauvais kind). Pas d'`edges` persistés (`CanvasState.edges` réservé/inutilisé). Géométries/logiques **pures** côté front (testées `node --test`) : `cables.js` (`MekiCables` — subway 45°, ruban, contournement, `pathBetween`), `collision.js` (`MekiCollision` — collision douce), `folders.js` (`MekiFolders` — chaîne de dossiers complète/compacte), `git-node.js` (`MekiGitNode` — rendu état git). `canvas.js` les câble en DOM impératif. Node git rafraîchie à la fin de tour (`GET /api/git/branch`, lecture seule). **Placement organique INCRÉMENTAL** des nodes dossier/fichier : chaque nouveau node est posé **une seule fois** dans un trou libre près de son parent, en croissant **vers l'extérieur** (dendrite, `editorSpawnPos`) ; les nodes existants **ne bougent jamais** (pas de re-layout global → pas de clignotement), `fitView` auto-zoome pour tout voir ; les dossiers sont **matérialisés par la comète** comme les fichiers.
