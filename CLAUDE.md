...
- Câbles/wires **dérivés** d'un parent par node (`Node.source_id`). Parentage **par kind** (`CANONICAL_PARENT_KIND`) pour les built-in, **path-aware** (plus-long-préfixe de chemin, `nodes/parenting.py` pur) pour `folder`/`fileeditor` ; `reconcile_source_links` applique les deux au boot (+ migration des built-in de mauvais kind). Pas d'`edges` persistés (`CanvasState.edges` réservé/inutilisé). Géométries/logiques **pures** côté front (testées `node --test`) : `cables.js` (`MekiCables` — subway 45°, ruban, contournement, `pathBetween`), `collision.js` (`MekiCollision` — collision douce), `folders.js` (`MekiFolders` — chaîne de dossiers complète/compacte), `git-node.js` (`MekiGitNode` — rendu état git), `neuro-layout.js` (`MekiNeuroLayout` — disposition organique « neurones » directionnelle). `canvas.js` les câble en DOM impératif. Node git rafraîchie à la fin de tour (`GET /api/git/branch`, lecture seule). Le sous-arbre dossiers→fichiers est disposé en **dendrites directionnelles** autour de l'explorateur (`layoutFolderTree`/`fitView`, anti-collision + anti-câble-sous-node) ; les dossiers sont **matérialisés par la comète** comme les fichiers.
